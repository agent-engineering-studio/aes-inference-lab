#!/usr/bin/env bash
# sysctl, scheduler I/O, file descriptor, wait-online. Idempotente.
...
sysctl --system >/dev/null && ok "sysctl applicato"
...
step "File descriptor"
install_file "$ROOT/etc/security/99-inference.conf" /etc/security/limits.d/99-inference.conf
warn "i limiti si applicano al prossimo login; i servizi usano LimitNOFILE nelle unit"
...
if systemctl is-enabled systemd-networkd-wait-online.service >/dev/null 2>&1; then
  ...
  ok "wait-online: procede appena una interfaccia e' online (timeout 30s)"
else
  ok "wait-online gia' disabilitato"
fi

ok "tuning completato"
